...
const MovieCardDescription = ({ movie }) => {
  return (
    <>
      <div className={classes.descriptionHeader}>
        ...
        <div className={classes.movieTitle}>{movie?.ru_title}</div>
      </div>
      <div className={classes.descriptionBody}>
        <div className={classes.movieDescription}>{movie?.kp?.description}</div>
        <div className={classes.movieAdditionalInformation}>
          <div>Год производства</div>
          <div>{movie.kp.year}</div>
          <div>Страна</div>
          <div>{movie?.kp?.countries.map((el) => el.country).join(", ")}</div>
          <div>Жанр</div>
          <div>{movie?.kp?.genres.map((el) => el.genre).join(", ")}</div>
